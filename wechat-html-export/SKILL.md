...
description: 微信公众号「月半里予」的 HTML 导出与排版规范子技能。提供公众号粘贴用 HTML 生成（一键复制、ClipboardItem text/html）、严格标签/CSS 白名单、灰度色号规范、图片规则、布局限制和输出复查清单。由主技能 wechat-yuebanliyu 在「导出 HTML」或「生成公众号 HTML」时自动调用。触发关键词：导出HTML、生成HTML、公众号粘贴、生成排版。
---

# 月半里予 · 公众号 HTML 导出与排版规范

> 本技能是 `wechat-yuebanliyu` 的子技能，专司微信公众号文章的 HTML 格式输出与排版质量控制。由主技能按需调用，也可独立触发。

---

## 一、触发时机

- 用户明确要求"导出为 HTML"、"生成 HTML 文件"、"我要粘贴到公众号"
- 用户说"做成浏览器打开就能复制的那种"
- 主技能 Step 8.5 流程流转至本技能
- 用户要求检查/修改排版规范
...
```
D:\工作文档-NAS\Lobster Working Well\月半里予公众号\编号_年月日_文章标题\
├── 文章标题.md
├── 配图提示词.md
└── 文章标题.html
```
...
- **Step 8.5**：用户要求导出 HTML 时调用
- **排版检查**：用户质疑排版规范时复查
- **非彩色文字色号**：所有色号判断以 3.3 节为准
